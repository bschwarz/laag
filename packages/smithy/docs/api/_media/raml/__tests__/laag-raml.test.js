...
beforeAll(() => {
  const examplePath = path.join(__dirname, '../examples/example.raml');
  let data = fs.readFileSync(examplePath, 'utf8');
  doc = new Raml(data);
  docNew = new Raml();
});
...
test('Get Display Name for an Operation', () => {
  expect(doc.getDisplayName('/organisation', 'get')).toBe('getOrganisation');
});
test('Set Display Name for an Operation', () => {
...
test('Get Operation Description', () => {
  ...
  expect(
    Object.keys(doc.getOperationDescription('/organisation', 'get')).length
