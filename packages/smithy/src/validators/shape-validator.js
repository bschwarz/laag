...
export class ShapeValidator {
    ...
    validate(shapeId, shape) {
        ...
        switch (shape.type) {
            case 'structure':
                result = this.validateStructure(shapeId, shape);
                break;
            case 'service':
                result = this.validateService(shapeId, shape);
                break;
            case 'operation':
                result = this.validateOperation(shapeId, shape);
                break;
            case 'resource':
                result = this.validateResource(shapeId, shape);
                break;
            case 'union':
                result = this.validateUnion(shapeId, shape);
                break;
            case 'list':
                result = this.validateList(shapeId, shape);
                break;
            case 'set':
                result = this.validateSet(shapeId, shape);
                break;
            case 'map':
                result = this.validateMap(shapeId, shape);
                break;
            default:
     ...
     * @returns Validation result
     */
    validateStructure(shapeId, shape) {
        const errors = [];
        // Validate members if present
        if (shape.members !== undefined) {
            if (typeof shape.members !== 'object' ||
                shape.members === null ||
     ...
     * @returns Validation result
     */
    validateOperation(shapeId, shape) {
        const errors = [];
     ...
     * @returns Validation result
     */
    validateResource(shapeId, shape) {
        const errors = [];
        ...
        const lifecycleOps = ['create', 'put', 'read', 'update', 'delete', 'list'];
        for (const op of lifecycleOps) {
            if (shape[op] !== undefined) {
                const opResult = this.validateShapeReference(shapeId, op, shape[op]);
                if (!opResult.valid) {
        ...
        if (shape.collectionOperations !== undefined) {
            const colOpsResult = this.validateShapeReferenceArray(shapeId, 'collectionOperations', shape.collectionOperations);
     ...
     * @returns Validation result
     */
    validateMember(shapeId, memberName, member) {
        const errors = [];
        if (!member.target) {
            errors.push({
     ...
     * @returns Validation result
     */
    validateShapeReference(shapeId, field, reference) {
        const errors = [];
     ...
     * @returns Validation result
     */
    validateShapeReferenceArray(shapeId, field, references) {
        const errors = [];
     ...
     * @returns Validation result
     */
    validateShapeReferenceMap(shapeId, field, references) {
        const errors = [];
...
//# sourceMappingURL=shape-validator.js.map