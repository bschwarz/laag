...
test('Get Operation Description', () => {
  expect(Object.keys(doc.getOperationDescription('/houses', 'post')).length).toBeGreaterThanOrEqual(
    5
  );
});
...
test('Delete an operation', () => {
  // Create a copy to avoid mutating the shared doc
  const docCopy = new Openapi(data);
  let res = docCopy.deleteOperation('/houses/{houseId}', 'delete');
  // console.log(res);
  expect(docCopy.operationExists('/houses/{houseId}', 'delete')).toBe(false);
});
test('Delete a path', () => {
  // Create a copy to avoid mutating the shared doc
  const docCopy = new Openapi(data);
  let res = docCopy.deletePath('/houses/{houseId}');
  // console.log(res);
  expect(docCopy.pathExists('/houses/{houseId}')).toBe(false);
});
